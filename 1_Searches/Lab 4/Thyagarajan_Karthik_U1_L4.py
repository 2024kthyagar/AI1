...
def inversion_count(new_state, width=4, N=4):
    ...
    # if N is even
        ...
        # if even row
        if ((len(new_state) - 1 - new_state.find('_')) // width) % 2 == 1:
...
def dist_heuristic(state, goal="123456789ABCDEF_", size=4):
    total_dist = 0
...
def check_heuristic():
    a = dist_heuristic("152349678_ABCDEF", "123456789ABCDEF_", 4)
    b = dist_heuristic("8936C_24A71FDB5E", "123456789ABCDEF_", 4)
    return a < b


def a_star(start, goal="123456789ABCDEF_", heuristic=dist_heuristic, size=4):
    # 2_63514B897ACDEF
