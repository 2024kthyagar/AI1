...
public class LunchRoom
{
   ...
   public static int[] numStudents = new int[4];
   public static int[] minStudents = new int[4];
   public static int[] waitStudents = new int[4];
   ...
   public static double calculateAverage(int mins, int customer)
   {
      return 1.0 * mins / customer;
   }
   ...
   public static void lunchRoom(int TIME, PrintWriter outfile)
   {
      ...
      *************************/
      PriorityQueue<Customer> hungry = new PriorityQueue<>();
      ...
      for(int min = 1; min <= TIME; min++)
      {
         ...
         { 
            ...
            int student = (int) ((Math.random()*4)+9);
            if(student == 12) {
               hungry.add(new Customer(min, servetime, student));
               numStudents[0]++;
               minStudents[0] += servetime;
            }
            else if(student == 11) {
               hungry.add(new Customer(min, servetime, student));
               numStudents[1]++;
               minStudents[1] += servetime;
            }
            else if(student == 10) {
               hungry.add(new Customer(min, servetime, student));
               numStudents[2]++;
               minStudents[2] += servetime;
            }
            else if(student == 9) {
               hungry.add(new Customer(min, servetime, student));
               numStudents[3]++;
               minStudents[3] += servetime;
            }
         }
         ...
         if(!hungry.isEmpty()) 
         {
            ...
            {
               Customer done = hungry.remove();
               if(done.getGrade() == 12) {
                  if(min - done.getArrive() > waitStudents[0])
                     waitStudents[0] = min-done.getArrive();
               }
               else if(done.getGrade() == 11) {
                  if(min - done.getArrive() > waitStudents[1])
                     waitStudents[1] = min-done.getArrive();
               }
               else if(done.getGrade() == 10) {
                  if(min - done.getArrive() > waitStudents[2])
                     waitStudents[2] = min-done.getArrive();
               }
               else if(done.getGrade() == 9) {
                  if(min - done.getArrive() > waitStudents[3])
                     waitStudents[3] = min-done.getArrive();
               }
      ...
      System.out.println("Senior\t\t" + numStudents[0] + "\t\t" + waitStudents[0] + "\t\t" + calculateAverage(waitStudents[0],numStudents[0]));
      System.out.println("Junior\t\t" + numStudents[1] + "\t\t" + waitStudents[1] + "\t\t" + calculateAverage(waitStudents[1],numStudents[1]));
      System.out.println("Sophomore\t\t" + numStudents[2] + "\t\t" + waitStudents[2] + "\t\t" + calculateAverage(waitStudents[2],numStudents[2]));
      System.out.println("Freshman\t\t" + numStudents[3] + "\t\t" + waitStudents[3] + "\t\t" + calculateAverage(waitStudents[3],numStudents[3]));
    ...
    ***********************************/
      public Customer(int a, int o, int g)
      {
         arrivedAt = a;
         orderAndBeServed = o;
         grade = g;
